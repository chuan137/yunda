...
def upload(request):
    ...
    if request.method == 'POST':
        ...
        c['form'] = form
        return render_to_response('cn_shenfenzheng/upload.html', c, context_instance=RequestContext(request))
...
def ajax_upload_image(request):
    if request.method == 'POST':
        ...
        img = request.FILES['image'] or False
        file_name = request.POST['filename'] or False
        
        if not file_name:
            while True:
                file_name = u"%s.jpg" % hashlib.md5(str(random.random()).encode('utf-8')).hexdigest()            
                file_url = u"%s%s%s" % (media_root, sfz_tmp_dir, file_name)
                file_path = u"%s%s" % (base_dir, file_url)
                if not os.path.isfile(file_path): break
        else:
            file_url = u"%s%s%s" % (media_root, sfz_tmp_dir, file_name)
            file_path = u"%s%s" % (base_dir, file_url)
        ...
        if width < 398 or height < 299:
            return HttpResponse(json.dumps({'result':False, 'info':u'图片太小，可能会不够清晰。图片最小需要大于500x320像素。'}), content_type="application/json")
        logger.error("aaa")
        img = image_resize_with_max_width(img, 800, 512)
        # add water mark
        font = ImageFont.truetype("%s%ssimsun.ttc" % (base_dir, media_root), 19)
        draw = ImageDraw.Draw(img)
        draw.text((30, 50), u"仅供中国海关清关使用", fill=(128, 128, 128), font=font)
        # end add water mark
        logger.debug(file_path)
        try:
            img.save("/tmp"+file_path, 'JPEG')
        except Exception as e:
            logger.error(e)
        finally:
            img.close()
        return HttpResponse(json.dumps({'result':True, 'info':file_url, 'filename':file_name}), content_type="application/json")       
