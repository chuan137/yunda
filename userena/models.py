...
from parcel.models import Level
from yunda_user.models import Branch, DepositTransferNew
...
class UserProfile(UserenaBaseProfile):
    user = models.OneToOneField(User,
                                unique=True,
                                verbose_name=_('User'),)
    customer_number = models.CharField(_('Customer number'), max_length=10, blank=True)
    ...
    salesman = models.ForeignKey(User, blank=True, null=True, related_name="UserProfile.salesman")
    ...
    current_deposit = models.FloatField(_('Current deposit (EUR)'), default=0)
    credit = models.FloatField(_('Credit (EUR)'), default=0)
    is_synced = models.NullBooleanField(default=False)
    level = models.ForeignKey(Level, null=True)
    allow_dhl_no_retoure=models.NullBooleanField(default=False)
    trusted=models.BooleanField(default=False)# allow customer to express customs clearance service

    def __unicode__(self):
        return 'Userprofile - %s - %s' % (self.customer_number, self.user)
    
    def deposit_deduct(self, amount, origin, ref):
        if amount < 0:
            return (False, "Amount less than 0")
        if amount < 0.01:
            return (True, "Success")
        if amount <= self.current_deposit + self.credit:
            try:
                DepositTransferNew.objects.create(amount=-amount, origin=origin, ref=ref, user_id=self.user_id)
                self.current_deposit = self.current_deposit - amount
                try:
                    self.save()
                    return (True, "Success")
                except:
                    DepositTransferNew.objects.create(amount=amount, origin=origin, ref=ref,user_id=self.user_id)
                    return (False, "Fail when updating deposit amount of this customer")
            except:
                return (False, "Fail when creating new deposit record")
        else:
            return (False, "Short in deposit")
    
    def deposit_increase(self, amount, origin, ref):
        if amount < 0:
            return (False, "Amount less than 0")
        if amount < 0.01: 
            return (True, "Success")
